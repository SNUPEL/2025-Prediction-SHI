...
def load_data(self):
    ...
    if pd.isna(pd.to_datetime(df_temp.columns[-1], errors='coerce')):
        ...
        df_temp.columns = new_cols
         ...
         df_sheet.columns = new_cols # 수정된 컬럼 이름 적용
    ...
    for i, row in df_temp.iterrows():
        ...
        config_start_date_dt = pd.to_datetime(self.config['data_start_date']).replace(day=1)
        r_start_date = max(actual_start_date, config_start_date_dt)
        ...
        r_condition = True # 모든 로딩된 회사를 일단 포함하는 것으로 간주

        self.company_dict[original_company_id] = Company(original_company_id, row[df_temp.columns[1]], row[df_temp.columns[2]], row[df_temp.columns[3]],
